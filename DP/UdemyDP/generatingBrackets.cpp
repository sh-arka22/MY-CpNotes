...
void generator(string &output,int n, int open, int close, int i){
	if(i==2*n){
		cout<<output<<endl;
		return;
	}
	...
	if(open<n){
		output.push_back('(');
		generator(output,n,open+1,close,i+1);
		output.pop_back();
	}
	...
	if(close<open){
		output.push_back(')');
		generator(output,n,open,close+1,i+1);
		output.pop_back();
	}
}
