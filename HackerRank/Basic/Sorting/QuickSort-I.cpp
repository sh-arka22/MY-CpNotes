...
int partition(vector<int>&arr,int lb, int up){
	int pivot = arr[lb];
	int start = lb, end = up;
...
void QickSort(vector<int>&arr, int lb, int ub){
	if(lb < ub){
		int middle = partition(arr,lb,ub);
		QickSort(arr,lb, middle-1);
		QickSort(arr, middle+1, ub);
	}
}
